...
'''.format(
    ...
    def extract_item(self, item, restore_attrs=True, dry_run=False, stdout=False, sparse=False,
        ...
        dest = self.cwd
        if item.path.startswith(('/', '../')) or (sys.platform == 'win32' and len(item.path) > 1 and item.path[1] == ':'):
    ...
    def restore_attrs(self, path, item, symlink=False, fd=None):
        ...
        if 'birthtime' in item:
            ...
            try:
                # This should work on FreeBSD, NetBSD, Darwin and Windows and be harmless on other platforms.
